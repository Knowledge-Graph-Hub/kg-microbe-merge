...
def remove_duplicate_nodes(con): #base_nodes_kg, subset_nodes_kg):

    con.execute("""
    ALTER TABLE base_nodes_kg ADD COLUMN source_table VARCHAR(20);
    UPDATE base_nodes_kg
    SET source_table = 'source_table';
    """)

    # # Insert data from the base table into the subset table
    # con.execute("""
    # INSERT INTO base_nodes_kg ({nodes_columns_str}, source_table)
    # SELECT {nodes_columns_str}, 'source_table'
    ...
    con.execute("""
        ...
        FROM (
            SELECT *,
                ROW_NUMBER() OVER (
                    PARTITION BY id
                    ORDER BY CASE WHEN source_table = 'source_table' THEN 1 ELSE 2 END
                ) as rn
            FROM final_kg
        ) sub
        WHERE sub.rn = 1;
...
def write_nodes(con):
    ...
    output_filename = os.getcwd() + "/merged_kg_nodes.tsv"
